...
import { readableStreamToString, LuccaOrganizationError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LuccaOrganizationError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LuccaOrganizationError(e, e.response?.data)
                }
                if (innerError instanceof LuccaOrganizationError) {
                    // Got "LuccaOrganizationError" from the above try block
                    throw innerError;
                }
